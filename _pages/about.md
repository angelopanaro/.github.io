---
permalink: /
title: ""
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

<br>
<br>

Welcome to my website! 

I am a postdoctoral researcher at the [Friedrich-Alexander University (FAU)](https://www.pol.phil.fau.eu/), where I collaborate with the [Varieties of Democracy (V-Dem) Institute](https://www.v-dem.net/) on the [Academic Freedom Index research project](https://academic-freedom-index.net/). Previously, I was an adjunct professor at the [University of Bologna](https://www.unibo.it/en), a postdoctoral researcher at [Bielefeld University](https://www.uni-bielefeld.de/(en)/), and a research fellow at the [University of Milan](https://www.unimi.it/en). I was also a visiting fellow at the [Research Center on Inequality and Social Policy (SOCIUM)](https://www.socium.uni-bremen.de/home/en/) at the University of Bremen, and the [European Centre for Social Welfare Policy and Research](https://www.euro.centre.org/)

My research focuses on the dynamics of regime change, social policy and welfare state reforms, and socio-economic inequality in authoritarian regimes. My work has been published in *Democratization*, *Contemporary Politics*, *Journal of International and Comparative Social Policy*, *International Journal of Educational Research*, *Review of Policy Research*, *Social Policies/Politiche Sociali*, *Italian Political Science Review*, among other journals. Since 2023, I have regularly contributed to the [Global Social Policy Digest](https://journals.sagepub.com/home/gsp).

I am currently serving as the coordinator of the [Italian Political Science (SISP) Standing Group ‘Political Regimes’](https://standinggroups.sisp.it/politicalregimes/) (with Dr. Tiziana Corda) and in 2022 I was awarded the [Carlo Maria Santoro Prize](https://www.sisp.it/premio-carlo-maria-santoro#:~:text=Il%20Premio%20annuale%20%E2%80%9CCarlo%20Maria,degli%20autori%20e%20delle%20autrici.) for the best paper presented by an early-career researcher at the SISP Conference (co-authored with Dr. Andrea Vaccaro). 

<br>

<div style="display: flex; justify-content: space-between; flex-wrap: wrap;">

  <div style="flex: 0 0 48%;">
    <strong>Research Interests</strong>
    <ul>
      <li>Authoritarian regimes and democratic backsliding</li>
      <li>Social policy and comparative welfare states</li>
      <li>Global redistribution and socio-economic inequalities</li>
      <li>Data analysis</li>
    </ul>
  </div>
<div style="flex: 0 0 48%;">
  <strong>Education</strong>
  <ul style="padding-left: 0; list-style: none;"> 
    <li style="margin-bottom: 0.8em;">
      <!-- Graduation cap icon -->
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 640 512" style="margin-right: 0.3em;">
      <path d="M623.1 136.9l-282.7-101.2c-13.73-4.91-28.7-4.91-42.43 0L16.05 136.9C6.438 140.4 0 149.6 0 160s6.438 19.65 16.05 23.09L76.07 204.6c-11.89 15.8-20.26 34.16-24.55 53.95C40.05 263.4 32 274.8 32 288c0 9.953 4.814 18.49 11.94 24.36l-24.83 149C17.48 471.1 25 480 34.89 480H93.11c9.887 0 17.41-8.879 15.78-18.63l-24.83-149C91.19 306.5 96 297.1 96 288c0-10.29-5.174-19.03-12.72-24.89c4.252-17.76 12.88-33.82 24.94-47.03l190.6 68.23c13.73 4.91 28.7 4.91 42.43 0l282.7-101.2C633.6 179.6 640 170.4 640 160S633.6 140.4 623.1 136.9zM351.1 314.4C341.7 318.1 330.9 320 320 320c-10.92 0-21.69-1.867-32-5.555L142.8 262.5L128 405.3C128 446.6 213.1 480 320 480c105.1 0 192-33.4 192-74.67l-14.78-142.9L351.1 314.4z"/>
      </svg>
    PhD in Political Science (2021)
      <div style="font-size: 0.85em; margin-left: 1.8em;">Scuola Normale Superiore</div>
    <li style="margin-bottom: 0.8em;">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 640 512" style="margin-right: 0.3em;">
        <path d="M623.1 136.9l-282.7-101.2c-13.73-4.91-28.7-4.91-42.43 0L16.05 136.9C6.438 140.4 0 149.6 0 160s6.438 19.65 16.05 23.09L76.07 204.6c-11.89 15.8-20.26 34.16-24.55 53.95C40.05 263.4 32 274.8 32 288c0 9.953 4.814 18.49 11.94 24.36l-24.83 149C17.48 471.1 25 480 34.89 480H93.11c9.887 0 17.41-8.879 15.78-18.63l-24.83-149C91.19 306.5 96 297.1 96 288c0-10.29-5.174-19.03-12.72-24.89c4.252-17.76 12.88-33.82 24.94-47.03l190.6 68.23c13.73 4.91 28.7 4.91 42.43 0l282.7-101.2C633.6 179.6 640 170.4 640 160S633.6 140.4 623.1 136.9zM351.1 314.4C341.7 318.1 330.9 320 320 320c-10.92 0-21.69-1.867-32-5.555L142.8 262.5L128 405.3C128 446.6 213.1 480 320 480c105.1 0 192-33.4 192-74.67l-14.78-142.9L351.1 314.4z"/>
      </svg>
    MA in Economics (2016)
      <div style="font-size: 0.85em; margin-left: 1.8em;">Bocconi University</div>
    <li style="margin-bottom: 0.8em;">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 640 512" style="margin-right: 0.3em;">
        <path d="M623.1 136.9l-282.7-101.2c-13.73-4.91-28.7-4.91-42.43 0L16.05 136.9C6.438 140.4 0 149.6 0 160s6.438 19.65 16.05 23.09L76.07 204.6c-11.89 15.8-20.26 34.16-24.55 53.95C40.05 263.4 32 274.8 32 288c0 9.953 4.814 18.49 11.94 24.36l-24.83 149C17.48 471.1 25 480 34.89 480H93.11c9.887 0 17.41-8.879 15.78-18.63l-24.83-149C91.19 306.5 96 297.1 96 288c0-10.29-5.174-19.03-12.72-24.89c4.252-17.76 12.88-33.82 24.94-47.03l190.6 68.23c13.73 4.91 28.7 4.91 42.43 0l282.7-101.2C633.6 179.6 640 170.4 640 160S633.6 140.4 623.1 136.9zM351.1 314.4C341.7 318.1 330.9 320 320 320c-10.92 0-21.69-1.867-32-5.555L142.8 262.5L128 405.3C128 446.6 213.1 480 320 480c105.1 0 192-33.4 192-74.67l-14.78-142.9L351.1 314.4z"/>
      </svg>
    BA in Political Science (2012)
      <div style="font-size: 0.85em; margin-left: 1.8em;">University of Bologna - Forlì campus</div>
    </li>
